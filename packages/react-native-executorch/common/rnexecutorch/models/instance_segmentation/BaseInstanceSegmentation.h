...
private:
  ...
  cv::Rect computeMaskCropRect(const cv_processing::BBox &bboxModel,
                               cv::Size modelInputSize, cv::Size maskSize);

  cv::Rect addPaddingToRect(const cv::Rect &rect, cv::Size maskSize);

  cv::Mat warpToOriginalResolution(const cv::Mat &probMat,
                                   const cv::Rect &maskRect,
                                   cv::Size originalSize, cv::Size maskSize,
                                   const cv_processing::BBox &bboxOriginal);
  ...
  cv::Mat processMaskFromLogits(const cv::Mat &logitsMat,
                                const cv_processing::BBox &bboxModel,
                                const cv_processing::BBox &bboxOriginal,
                                cv::Size modelInputSize, cv::Size originalSize,
                                bool warpToOriginal);
